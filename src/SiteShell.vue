...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('scientificObjects.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('scientificObjects.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('scientificObjects.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
